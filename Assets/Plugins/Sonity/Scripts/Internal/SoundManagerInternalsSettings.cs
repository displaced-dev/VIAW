...
namespace Sonity.Internal {
    ...
    public class SoundManagerInternalsSettings {
        ...
        public bool soundManagerManualUpdate = false;

        public bool playOneShotOptimization = false;
        ...
        // SoundEvent Library
        public bool soundEventLibraryEnable = false;

        // Entity Sound
        public bool entitySoundBaseEnable = false;
        public bool entitySoundLibraryEnable = false;
        ...
        public IEnumerator LoadAddressableAudioMixerAsync() {
            if (!addressableAudioMixerUse) {
                Debug.LogError($"Sonity.{NameOf.SoundManager}: Addressable AudioMixer is disabled but still is trying to be used.");
                yield break;
            }
            ...
            if (addressableAudioMixerReference == null) {
                Debug.LogError($"Sonity.{NameOf.SoundManager}: Addressable AudioMixer AssetReference is null.");
                yield break;
            }

            if (addressableAudioMixerReference.RuntimeKeyIsValid() == false) {
                Debug.LogError($"Sonity.{NameOf.SoundManager}: Addressable AudioMixer AssetReference runtime key is not valid.");
                yield break;
            }
            ...
            if (audioMixerLoadHandle.Status == AsyncOperationStatus.Succeeded) {
                ...
                Debug.LogError($"Sonity.{NameOf.SoundManager}: Addressable AudioMixer loading: async operation status failed.");
            } else if (audioMixerLoadHandle.Status == AsyncOperationStatus.None) {
                Debug.LogError($"Sonity.{NameOf.SoundManager}: Addressable AudioMixer loading: async operation status none.");
            }
        }
        ...
        public void LoadAddressableAudioMixer() {
                    ...
                    if (addressableAudioMixerReference != null && addressableAudioMixerReference.RuntimeKeyIsValid()) {
                        ...
                        if (audioMixerLoadHandle.Status == AsyncOperationStatus.Succeeded) {
                            if (audioMixerLoadHandle.Result != null) {
                                addressableAudioMixerAsset = audioMixerLoadHandle.Result;
                                SoundManagerBase.Instance.InternalAddressableAudioMixerInvoke(true);
                                return;
                            } else {
                                Debug.LogError($"Sonity.{NameOf.SoundManager}: Addressable AudioMixer is loaded but null.");
                            }
                        } else {
                            Debug.LogError($"Sonity.{NameOf.SoundManager}: Addressable AudioMixer AssetReference {addressableAudioMixerReference} failed to load.");
                        }
                    } else {
                        Debug.LogError($"Sonity.{NameOf.SoundManager}: Addressable AudioMixer AssetReference is null.");
                    }
            ...
            SoundManagerBase.Instance.InternalAddressableAudioMixerInvoke(false);
        }
#endif

        // Kept for legacy support so the old single SoundTag can be resaved in the array added in Sonity 1.1.2
        public SoundTagBase globalSoundTag;
        public List<SoundTagBase> globalSoundTags = new List<SoundTagBase>() {
            null,
        };

        public bool globalSoundTagsExpanded = true;
        ...
        public bool voiceEffectDisable = false;
    }
}