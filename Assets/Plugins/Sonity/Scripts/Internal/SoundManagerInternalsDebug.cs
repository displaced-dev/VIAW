...
namespace Sonity.Internal {
    ...
    public class SoundManagerInternalsDebug {
        ...
        public bool GetShouldDebugGlobal() {
            if (forceDebug) {
                return true;
            } else {
                return SonityEditorPrefs.SonitySoundManagerDebugEnable;
            }
        }
        ...
        public bool logSoundEventsSoundParametersContinuous = false;
        ...
        public bool GetDrawEnabledSceneView() {
            if (!GetShouldDebugGlobal()) {
                return false;
            }
            return drawSoundEventsInSceneViewEnabled;
        }

        public bool GetDrawEnabledGameView() {
            if (!GetShouldDebugGlobal()) {
                return false;
            }
            return drawSoundEventsInGameViewEnabled;
        }
        ...
        public void LogSoundEventsPlay(SoundEventBase soundEvent, SoundEventPlayType soundEventPlayType, Transform transformOwner, Transform transformPosition, Vector3? vector3Position) {
            ...
            if (soundEventPlayType == SoundEventPlayType.Play) {
                DebugLogPrint($"Sonity Log - <color=#62ff00>Play</color>: {soundEvent.internals.cachedName} at Owner \"{transformOwnerName}\"", selectedObject);
            } else if (soundEventPlayType == SoundEventPlayType.PlayAtVector) {
                DebugLogPrint($"Sonity Log - <color=#62ff00>Play</color>: {soundEvent.internals.cachedName} at Owner \"{transformOwnerName}\" and at Vector3 position {vector3Position.Value}", selectedObject);
                ...
                DebugLogPrint($"Sonity Log - <color=#62ff00>Play</color>: {soundEvent.internals.cachedName} at Owner \"{transformOwnerName}\" and at Transform position \"{transformPositionName}\"", selectedObject);
            }
        }
        ...
        public void LogSoundEventsStop(SoundEventBase soundEvent, bool allowFadeOut) {
            if (allowFadeOut) {
                DebugLogPrint($"Sonity Log - <color=red>Stop Allow Fadeout</color>: {soundEvent.internals.cachedName}", soundEvent);
            } else {
                DebugLogPrint($"Sonity Log - <color=red>Stop Immediate</color>: {soundEvent.internals.cachedName}", soundEvent);
            }
        }
        ...
        public void LogSoundEventsPool(SoundEventBase soundEvent) {
            DebugLogPrint($"Sonity Log - <color=#bf00ff>Pool</color>: {soundEvent.internals.cachedName}", soundEvent);
        }
        ...
        public void LogSoundEventsPauseUnpause(SoundEventBase soundEvent, bool pause) {
            if (pause) {
                DebugLogPrint($"Sonity Log - <color=orange>Pause</color>: {soundEvent.internals.cachedName}", soundEvent);
            } else {
                DebugLogPrint($"Sonity Log - <color=orange>Unpause</color>: {soundEvent.internals.cachedName}", soundEvent);
            }
        }
        ...
        public void LogSoundEventsGlobalPauseUnpause(SoundEventBase soundEvent, bool pause) {
            if (pause) {
                DebugLogPrint($"Sonity Log - <color=orange>Global Pause</color>: {soundEvent.internals.cachedName}", soundEvent);
            } else {
                DebugLogPrint($"Sonity Log - <color=orange>Global Unpause</color>: {soundEvent.internals.cachedName}", soundEvent);
            }
        }
        ...
        public bool LogSoundEventsSoundParametersContinuousEnabled(SoundEventBase soundEvent) {
            if (!GetShouldDebugGlobal()) {
                return false;
            }
            if (soundEvent.internals.data.debugLogShow == DebugSoundEventLogShow.Show) {
                return logSoundEventsEnabled && logSoundEventsSoundParametersContinuous;
            } else if (soundEvent.internals.data.debugLogShow == DebugSoundEventLogShow.Hide) {
                return false;
            } else if (soundEvent.internals.data.debugLogShow == DebugSoundEventLogShow.ForceShow) {
                return logSoundEventsSoundParametersContinuous;
            }
            return false;
        }
        ...
        public void LogSoundEventsSoundParametersText(SoundEventBase soundEvent, SoundParameterType soundParameterType, UpdateMode updateMode, string value) {
            if ((logSoundEventsSoundParametersOnce && updateMode == UpdateMode.Once) || (logSoundEventsSoundParametersContinuous && updateMode == UpdateMode.Continuous)) {
                DebugLogPrint($"Sonity Log - <color=yellow>Set SoundParameter{soundParameterType} {updateMode}</color>: {soundEvent.internals.cachedName} to: {value}", soundEvent);
            }
        }
        ...
        public void LogSoundEventsSoundParameters(SoundEventBase soundEvent, SoundParameterInternals[] soundParameters) {
            ...
            for (int i = 0; i < soundParameters.Length; i++) {
                if (soundParameters[i] != null) {
                    ...
                    // Force 2D
                    else if (soundParameterType == SoundParameterType.Force2D) {
                        LogSoundEventsSoundParametersText(soundEvent, soundParameterType, updateMode, soundParameters[i].internals.valueBool.ToString());
                    }
                    // Bypass Reverb Zones
