...
namespace Sonity.Internal {
    ...
    public class VoiceParameterInstance {
        ...
        // This is the part you need to check Eg: value and valueUse
        // Is updated every update
        ...
        public bool CanHaveFollowPosition() {
            // First check once modifiers
            if (currentModifier.followPositionUse && currentModifier.followPosition) {
                return true;
            }
            // Then Check Continuous Parameters (if it can be activated later)
            if (soundParameters != null && soundParameters.Length > 0) {
                for (int i = 0; i < soundParameters.Length; i++) {
                    if (soundParameters[i] != null && soundParameters[i].internals.type == SoundParameterType.FollowPosition) {
            ...
            if (soundParameters == null || soundParameters.Length == 0) {
                ...
                for (int i = 0; i < soundParameters.Length; i++) {
                    if (soundParameters[i] != null && soundParameters[i].internals.updateMode == UpdateMode.Once) {
                        ...
                        // Force 2D
                        else if (soundParameters[i].internals.type == SoundParameterType.Force2D) {
                            offsetModifier.force2D = soundParameters[i].internals.valueBool;
                            offsetModifier.force2DUse = true;
                        }
                        // Bypass Reverb Zones
