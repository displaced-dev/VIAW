...
namespace Sonity.Internal {
    ...
    public class SoundContainerInternals {
        ...
        public void LoadUnloadAudioData(bool load, SoundContainerBase parent) {
            if (audioClips.Length == 0) {
                if (ShouldDebug.Warnings()) {
                    Debug.LogWarning($"{NameOf.SoundContainer} {parent.internals.cachedName} has no {nameof(AudioClip)}.", parent);
                }
            } else {
                for (int i = 0; i < audioClips.Length; i++) {
                    if (audioClips[i] == null) {
                        if (ShouldDebug.Warnings()) {
                            Debug.LogWarning($"{NameOf.SoundContainer} {parent.internals.cachedName} {nameof(AudioClip)} {i} is null.", parent);
                        }
                    } else {
        ...
        public int GetRandomClip() {
            ...
            for (int i = 0; i < 40; i++) {
                ...
                if (!RandomClipLastContains(randomClip)) {
                    break;
                }
            }
    ...
    public class SoundContainerInternalsData {
        ...
        public bool noPlayOneShotOptimization = false;
