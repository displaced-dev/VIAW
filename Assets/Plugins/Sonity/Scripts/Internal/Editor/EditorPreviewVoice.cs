    ...
    public class EditorPreviewVoice {
        ...
        public void UpdateVoice() {
            if (previewSoundContainerSetting != null && previewSoundContainerSetting.soundContainer != null) {
                ...
                bool force2DEnabled = soundEventModifier.force2DUse && soundEventModifier.force2D;

                if (previewSoundContainerSetting.soundContainer.internals.data.distanceEnabled && !force2DEnabled) {
                ...
                // Spatialize in preview makes the sound not output if the application is not running
                ...
                // Is it a SoundEvent or SoundContainer we're previewing?
                PreviewObjectType previewObject = PreviewObjectType.SoundEvent;
                // SE is null when previewing SC
                if (previewSoundContainerSetting.soundEvent == null) {
                    previewObject = PreviewObjectType.SoundContainer;
                }

                // SoundEvent Volume
                if (previewObject == PreviewObjectType.SoundEvent) {
                ...
                // AudioMixerGroup
                if (previewObject == PreviewObjectType.SoundContainer) {
