...
namespace Sonity.Internal {
	...
	public static class EditorDragAndDropArea {
		...
		public static void DrawDragAndDropAreaCustomEditor<T>(DragAndDropAreaInfo dragAreaInfo, System.Action<T[]> OnPerformedDragCallback = null, bool searchSubFolders = false) where T : UnityEngine.Object {
        ...
        private static T[] GetDraggedObjects<T>() where T : UnityEngine.Object {
			...
			foreach (var draggedObject in DragAndDrop.objectReferences) {
				...
				if (draggedObject is DefaultAsset) {
					...
					T[] assetsInDraggedFolders = GetAllAssetsOfTypeInDirectory<T>(folderPath, true);
        ...
        public static T[] GetAllAssetsOfTypeInDirectory<T>(string path, bool searchSubfolders) where T : UnityEngine.Object {
            List<T> assetsToGet = new List<T>();
            if (searchSubfolders) {
                return FindAllAssetsOfTypeInPath<T>(path);
            } else {
                string absolutePath = $"{Application.dataPath}/{path.Remove(0, 7)}";
                string[] fileEntries = new string[0];
                if (searchSubfolders) {
                    fileEntries = Directory.GetFiles(absolutePath, "*", SearchOption.AllDirectories);
                } else {
                    fileEntries = Directory.GetFiles(absolutePath);
                }
