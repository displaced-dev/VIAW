...
namespace Sonity.Internal {
    ...
    public abstract class SoundEventEditorBase : Editor {
                    ...
                    if (soundPreset != null && !soundPreset.internals.disableAll) {
                        for (int ii = 0; ii < SoundPresetFind.soundPresets[i].internals.soundPresetGroup.Length; ii++) {
                            ...
                            if (soundPresetGroup != null && !soundPresetGroup.disable && soundPresetGroup.soundEventPreset != null) {
                                ...
                                string parentName = $"Preset - {soundPresetGroup.soundEventPreset.internals.cachedName}";
                                ...
                                for (int ii = 0; ii < soundPreset.internals.soundPresetGroup.Length; ii++) {
                                    ...
                                    if (soundPresetGroup != null && !soundPresetGroup.ShouldUseMatch(false)) {
                                        for (int iii = 0; iii < mTargets.Length; iii++) {
                                            if (soundPresetGroup.GetNameMatches(mTargets[iii].internals.cachedName, false)) {
                                                ...
                                                Debug.Log($"Sonity: Preset \"{soundPresetGroup.soundEventPreset.internals.cachedName}\" is applied to \"{mTargets[iii].internals.cachedName}\"", mTargets[iii]);
        ...
        private void GuiSpatializationInfo() {
            ...
            // Force 2D is only for SoundEvents
            List<SoundEventBase> soundsForce2d = new List<SoundEventBase>();
            // Sounds mixed is only when there are multiple SoundContainers with different values
            ...
            for (int i = 0; i < mTargets.Length; i++) {
                ...
                // Force 2D
                if (soundEvent.internals.data.soundEventModifier.force2DUse && soundEvent.internals.data.soundEventModifier.force2D) {
                    if (!soundsForce2d.Contains(soundEvent)) {
                        soundsForce2d.Add(soundEvent);
                    }
                    // Force 2D overrides the other settings
                    continue;
                }
                ...
                if (soundTypeCountAdded > 1) {
                    ...
                    // Force 2D is not added here, this is only mixed SoundContainers
                }
            ...
            } else if (soundsForce2d.Count > 0) {
                // Force 2D, use the blue 2d color
                StartBackgroundColor(EditorColor.GetSound2dColor(EditorColorProSkin.GetCustomEditorBackgroundAlpha()));
            ...
            if (soundsForce2d.Count > 0) {
                EditorGUILayout.BeginHorizontal();
                if (soundsForce2d.Count == 1) {
                    EditorGUILayout.LabelField(new GUIContent(EditorTextSoundEvent.spatializationInfoForce2DSoundSingleLabel, EditorTextSoundEvent.spatializationInfoForce2DSoundTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                } else {
                    EditorGUILayout.LabelField(new GUIContent($"{EditorTextSoundEvent.spatializationInfoForce2DSoundMultipleLabel} ({soundsForce2d.Count})", EditorTextSoundEvent.spatializationInfoForce2DSoundTooltip), GUILayout.Width(EditorGUIUtility.labelWidth));
                }
                if (multipleTypesSelected && mTargets.Length > 1) {
                    BeginChange();
                    if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSELabel, EditorTextSoundEvent.spatializationInfoSelectSETooltip), GUILayout.Width(buttonLabelWidthThird))) {
                        AssetDatabase.SaveAssets();
                        Selection.objects = soundsForce2d.ToArray();
                        AssetDatabase.SaveAssets();
                    }
                    EndChange();
                } else {
                ...
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoSelectSCLabel, EditorTextSoundEvent.spatializationInfoSelectSCTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    SelectSoundContainersInSoundEvents(soundsForce2d);
                }
                EndChange();
                // No Force 2D
                BeginChange();
                if (GUILayout.Button(new GUIContent(EditorTextSoundEvent.spatializationInfoNoForce2DLabel, EditorTextSoundEvent.spatializationInfoNoForce2DTooltip), GUILayout.Width(buttonLabelWidthThird))) {
                    for (int i = 0; i < soundsForce2d.Count; i++) {
                        SoundEventBase soundEvent = soundsForce2d[i];
                        Undo.RecordObject(soundEvent, "Remove Force 2D");
                        soundEvent.internals.data.soundEventModifier.force2DUse = false;
                        soundEvent.internals.data.soundEventModifier.force2D = false;
        ...
        private void GuiSoundContainers() {
            ...
            if (expandSoundContainers.boolValue) {
                ...
                EditorDragAndDropArea.DrawDragAndDropAreaCustomEditor<SoundContainerBase>(new EditorDragAndDropArea.DragAndDropAreaInfo($"{NameOf.SoundContainer}"), DragAndDropCallback);
            }
        ...
        private void UpdateModifier(SerializedProperty soundEventModifier) {
            ...
            // Force 2D
            if (soundEventModifier.FindPropertyRelative(nameof(SoundEventModifier.force2DUse)).boolValue) {
                SerializedProperty force2D = soundEventModifier.FindPropertyRelative(nameof(SoundEventModifier.force2D));
                BeginChange();
                EditorGUILayout.PropertyField(force2D, new GUIContent(EditorTextModifier.force2DLabel, EditorTextModifier.force2DTooltip));
                EndChange();
            }
            ...
            if (ShouldDebug.GuiWarnings()) {
                if (triggerOnPlayEnable.boolValue) {
                    if (mTarget.internals.data.GetIfInfiniteLoop(mTarget, out SoundEventBase infiniteInstigator, out SoundEventBase infinitePrevious, TriggerOnType.TriggerOnPlay)) {
                        EditorGUILayout.HelpBox($"Trigger On Play: \"{infiniteInstigator.internals.cachedName}\" in \"{infinitePrevious.internals.cachedName}\" creates an infinite loop", MessageType.Error);
                        EditorGUILayout.Separator();
                    }
                }
            }
            ...
            if (ShouldDebug.GuiWarnings()) {
                if (triggerOnStopEnable.boolValue) {
                    if (mTarget.internals.data.GetIfInfiniteLoop(mTarget, out SoundEventBase infiniteInstigator, out SoundEventBase infinitePrevious, TriggerOnType.TriggerOnStop)) {
                        EditorGUILayout.HelpBox($"Trigger On Stop: \"{infiniteInstigator.internals.cachedName}\" in \"{infinitePrevious.internals.cachedName}\" creates an infinite loop", MessageType.Error);
                        EditorGUILayout.Separator();
                    }
                }
            }
                ...
                if (ShouldDebug.GuiWarnings()) {
                    if (mTarget.internals.data.GetIfInfiniteLoop(mTarget, out SoundEventBase infiniteInstigator, out SoundEventBase infinitePrevious, TriggerOnType.TriggerOnTail)) {
                        EditorGUILayout.HelpBox(
                            $"Trigger On Tail: \"{infiniteInstigator.internals.cachedName}\" in \"{infinitePrevious.internals.cachedName}\" creates an infinite loop\n" +
        ...
        private void GuiSoundTag() {
            ...
            if (expandAllSoundTag.boolValue && soundTagEnable.boolValue) {
                ...
                for (int i = 0; i < soundTagGroups.arraySize; i++) {
                    ...
                    SerializedProperty soundTag = soundEventSoundTagGroupElement.FindPropertyRelative(nameof(SoundTagGroup.soundTag));
                    SerializedProperty baseSoundEventModifier = soundEventSoundTagGroupElement.FindPropertyRelative(nameof(SoundTagGroup.soundEventModifierBase));
                    SerializedProperty soundTagSoundEventModifier = soundEventSoundTagGroupElement.FindPropertyRelative(nameof(SoundTagGroup.soundEventModifierSoundTag));
                    SerializedProperty soundEvent = soundEventSoundTagGroupElement.FindPropertyRelative(nameof(SoundTagGroup.soundEvent));
                    EditorGUILayout.BeginHorizontal();
