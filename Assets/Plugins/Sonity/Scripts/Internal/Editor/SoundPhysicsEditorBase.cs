        ...
        private void LegacyAssignSoundEventsToArray() {
            ...
            for (int i = 0; i < mTargets.Length; i++) {
                ...
                if (anyAdded) {
                    ...
                    Debug.LogWarning($"Sonity.{NameOf.SoundPhysics}: Converted Legacy SoundEvent to Array - {mTargets[i].gameObject.name}");
                }
            }
        }
                    ...
                    for (int i = 0; i < impactSoundPhysicsParts.arraySize; i++) {
                        ...
                        if (GUILayout.Button("Reset")) {
                            for (int ii = 0; ii < mTargets.Length; ii++) {
                                Undo.RecordObject(mTargets[ii], $"Reset {NameOf.SoundPhysics}");
