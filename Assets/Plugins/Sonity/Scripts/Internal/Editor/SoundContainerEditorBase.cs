...
namespace Sonity.Internal {
    ...
    public abstract class SoundContainerEditorBase : Editor {
        ...
        public SerializedProperty noPlayOneshotOptimization;
        ...
        private void FindProperties() {
            ...
            noPlayOneshotOptimization = data.FindPropertyRelative(nameof(SoundContainerInternalsData.noPlayOneShotOptimization));
        ...
        public override void OnInspectorGUI() {
            ...
            if (ShouldDebug.GuiWarnings()) {
                // Waring if Disable Voice Effects is active
                    EditorGUILayout.Separator();
                    EditorGUILayout.HelpBox(EditorTextSoundContainer.disableVoiceEffectsWarning, MessageType.Warning);
                    EditorGUILayout.Separator();
            }
#endif
            GuiDistortion();
                    ...
                    if (soundPreset != null && !soundPreset.internals.disableAll) {
                        for (int ii = 0; ii < SoundPresetFind.soundPresets[i].internals.soundPresetGroup.Length; ii++) {
                            ...
                            if (soundPresetGroup != null && !soundPresetGroup.disable && soundPresetGroup.soundContainerPreset != null) {
                                ...
                                string parentName = $"Preset - {soundPresetGroup.soundContainerPreset.internals.cachedName}";
            ...
            try {
                ...
                if (menuObject.presetType == PresetType.SFX3D) {
                    ...
                    for (int i = 0; i < mTargets.Length; i++) {
                        ...
                        if (mTargets[i].internals.cachedName.ToLowerInvariant().Contains("loop")) {
                    ...
                    for (int i = 0; i < targets.Length; i++) {
                        string tempName = mTargets[i].internals.cachedName;
                                ...
                                for (int ii = 0; ii < soundPreset.internals.soundPresetGroup.Length; ii++) {
                                    ...
                                    if (soundPresetGroup != null && !soundPresetGroup.ShouldUseMatch(true)) {
                                        for (int iii = 0; iii < mTargets.Length; iii++) {
                                            if (soundPresetGroup.GetNameMatches(mTargets[iii].internals.cachedName, true)) {
                                                ...
                                                if (soundPreset.internals.automaticLoop && mTargets[iii].internals.cachedName.ToLowerInvariant().Contains("loop")) {
                                                ...
                                                Debug.Log($"Sonity: Preset \"{soundPresetGroup.soundContainerPreset.internals.cachedName}\" is applied to \"{mTargets[iii].internals.cachedName}\"", mTargets[iii]);
        ...
        private void GuiSettingsBase() {
            ...
            if (settingsExpandBase.boolValue) {
                ...
                if (settingsExpandAdvanced.boolValue) {
                    ...
                    // Disable Play Oneshot Optimization
                    BeginChange();
                    EditorGUILayout.PropertyField(noPlayOneshotOptimization, new GUIContent(EditorTextSoundContainer.noPlayOneShotOptimizationLabel, EditorTextSoundContainer.noPlayOneShotOptimizationTooltip));
                    EndChange();
                }
