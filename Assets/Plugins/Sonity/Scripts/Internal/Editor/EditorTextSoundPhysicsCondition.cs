...
namespace Sonity.Internal {
    ...
    public class EditorTextSoundPhysicsCondition {
        ...
        public static readonly string soundPhysicsConditionTooltip =
            $"{NameOf.SoundPhysicsCondition} objects are used in the {NameOf.SoundPhysics} component to decide if a physics interaction should play a {NameOf.SoundEvent} or not." + "\n" +
            "\n" +
            ...
            "\n" +
            $"You can also use them for playing different sounds with a single {NameOf.SoundEvent} by using SoundTags." + "\n" +
            "\n" +
            $"All {NameOf.SoundPhysicsCondition} objects are multi-object editable." + EditorTrial.trialTooltip;

        public static readonly string childrenLabel = $"Children";
        public static readonly string childrenTooltip =
            $"You can nest {NameOf.SoundPhysicsCondition}s so you don't have to update all your {NameOf.SoundPhysics} components when adding a new {NameOf.SoundPhysicsCondition}." + "\n" +
            "\n" +
            $"The children of the parent {NameOf.SoundPhysicsCondition} are evaluated after the parent." + EditorTrial.trialTooltip;

        public static readonly string parentLabel = $"Parent";
        public static readonly string parentTooltip = $"The parent {NameOf.SoundPhysicsCondition} is evaluated before the children." + EditorTrial.trialTooltip;


        public static readonly string soundTagLabel = $"SoundTag";
        public static readonly string soundTagTooltip =
            $"By utilizing the {NameOf.SoundTag}s you can play different sounds with a single {NameOf.SoundEvent} (see the example physics assets on how you can set that up)." + "\n" +
            "\n" +
            $"If no {NameOf.SoundTag} is assigned it won't affect the output {NameOf.SoundTag}, so if a child has an assigned {NameOf.SoundTag} and the parent doesn't, the child one will be used." + "\n" +
            "\n" +
            $"They are useful to split into different child {NameOf.SoundPhysicsCondition}s with different {NameOf.SoundTag}." + EditorTrial.trialTooltip;
        ...
        public static readonly string playDisregardingConditionsTooltip =
            $"If enabled the {NameOf.SoundPhysicsCondition} will play regardless of its own conditions." + "\n" +
            "\n" +
            $"Useful to combine with child {NameOf.SoundPhysicsCondition}s with different {NameOf.SoundTag}." + EditorTrial.trialTooltip;
        ...
        public static readonly string terrainNameHeaderTooltip =
            ...
            "\n" +
            $"The center point of the {NameOf.SoundPhysics} object is used to calculate the dominant Terrain Layer because it is more stable than the contacts." + EditorTrial.trialTooltip;
