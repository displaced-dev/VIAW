    ...
    public class SoundPhysicsInternals {
                ...
                if (isSub) {
                    if (ShouldDebug.Warnings()) {
                        if (conditionsIsNullDebugOnce == false) {
                            ...
                            Debug.LogWarning($"Sonity.{NameOf.SoundPhysicsCondition} is null in \"{parentCondition.internals.cachedName}\".", parentCondition);
                        }
                    }
                } else {
                    if (ShouldDebug.Warnings()) {
                        if (conditionsIsNullDebugOnce == false) {
                            conditionsIsNullDebugOnce = true;
                            Debug.LogWarning($"Sonity.{NameOf.SoundPhysics} Condition is null on \"{cachedTransform.name}\".", cachedTransform);
                        }
                    }
                }
#endif
        ...
        private int GetHighestImpulseContactIndex(Collision collision) {
            ...
            for (int n = 0; n < collision.contactCount; n++) {
                ...
                if (contactValue < velocity) {
                    contactValue = velocity;
                    contactIndex = n;
                }
            }
            return contactIndex;
