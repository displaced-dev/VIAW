                                    ...
                                    for (int iii = 0; iii < newAssetsList.Count; iii++) {
                                        ...
                                        if (newAsset != null) {
                                            ...
                                            if (soundContainer != null) {
                                                if (soundPresetGroup.GetNameMatches(soundContainer.internals.cachedName, true)) {
                                                    SoundContainerCopy.CopyTo(soundContainer, soundPresetGroup.soundContainerPreset);
                                                    if (SoundPresetFind.soundPresets[i].internals.automaticLoop && soundContainer.internals.cachedName.ToLowerInvariant().Contains("loop")) {
                                                    ...
                                                    Debug.Log($"Sonity: Preset \"{soundPresetGroup.soundContainerPreset.internals.cachedName}\" is applied to \"{soundContainer.internals.cachedName}\"", soundContainer);
                                                }
                                            }
                                        }
                                    }
                                ...
                                if (soundPresetGroup.ShouldUseMatch(false) && typeof(typeTo) == typeof(SoundEventBase)) {
                                    for (int iii = 0; iii < newAssetsList.Count; iii++) {
                                        ...
                                        if (newAsset != null) {
                                            ...
                                            if (soundEvent != null) {
                                                if (soundPresetGroup.GetNameMatches(soundEvent.internals.cachedName, false)) {
                                                    SoundEventCopy.CopyTo(soundEvent, soundPresetGroup.soundEventPreset);
                                                    EditorUtility.SetDirty(soundEvent);
                                                    Debug.Log($"Sonity: Preset \"{soundPresetGroup.soundEventPreset.internals.cachedName}\" is applied to \"{soundEvent.internals.cachedName}\"", soundEvent);
                                                }
                                            }
                                        }
                                    }
        ...
        private static void MakeFiles<typeFrom, typeTo>(List<string> rightTypeGuidList, bool makeMultipleAssets, bool removeTrailingNumbers, bool selectNewAssets) {
            ...
            for (int i = 0; i < rightTypeGuidList.Count; i++) {
                ...
                // From SoundEvent to SoundPolyGroup
                    ...
                    nameToRemove = GetNameToRemoveWithRightSeparatorChar(selectedFilename, "SE");
                    nameToAdd = $"{addSeparatorChar}POLY";
                }
                ...
                // From SoundContainer to SoundEvent
                    ...
                    if (makeMultipleAssets) {
                        ...
                        newAsset.internals.InitializeTimelineSoundContainerSetting();
                    }
                    // Make single asset
                    else {
                        ...
                        if (rightTypeGuidList.Count > 1) {
                            ...
                            while (charsEndToRemove <= selectedFilename.Length) {
                                ...
                                for (int ii = 0; ii < rightTypeGuidList.Count; ii++) {
                                    if (Path.GetFileNameWithoutExtension(AssetDatabase.GUIDToAssetPath(rightTypeGuidList[ii])).ToLowerInvariant().Contains(tempFilename.ToLowerInvariant())) {
                                        foundMatches++;
                                    }
                                }
